...
QUEFTS_WLYCY_Potato <- function(SoilData, WLY){	
 
  SoilData$water_limited_yield <- WLY
  Quefts_Input_Data_wly <- SoilData
  
  crop_param <- data.frame(aN=41, dN=96, aP=233, dP=588,  aK=34, dK=161, rN=0,rP=0, rK=0, max_yield = WLY, tolerance = 0.01 )
  
  
  #crop_param <- data.frame(aN=175, dN=435, aP=625, dP=4348,  aK=109, dK=513, rN=0,rP=0, rK=0, max_yield = WLY, tolerance = 0.01 )
  
  ## 1. get soil nutrient supply
  dss <- cbind(Quefts_Input_Data_wly, crop_param)	
  ...
  return(cbind(SoilData, data.frame(Current_Yield= getcy)))
...
QUEFTS1_Pedotransfer_potato <- function(QID, rec){		
  QID$WLY <- QID$water_limited_yield
...
NPK_TargetYield_forOutput_Maize <- function(df, rec) {
  NutrUse_soilNPK <- merge(df, rec)
  input <- NutrUse_soilNPK
  
  
  ## Supply of nutrients to the crop
  NutrUse_soilNPK$SN <- NutrUse_soilNPK$N + NutrUse_soilNPK$soilN
  NutrUse_soilNPK$SP <- NutrUse_soilNPK$P + NutrUse_soilNPK$soilP
  NutrUse_soilNPK$SK <- NutrUse_soilNPK$K + NutrUse_soilNPK$soilK
  
  ## Actual Uptake of nutrients: crop param + nutrient supply
  NutrUse_soilNPK <- actual_uptake_tool(NutrUse_soilNPK)
  
  ## max and min yield: actual uptake and crop param. min of N uptake constrianed by availability of P, K and water
  NutrUse_soilNPK <- max_min_yields_tools(NutrUse_soilNPK)
  
  ## final yield: min yield for combined uptake of 2 nutrients assuming the 3rd is not limiting, should be < WLY, and take meanof the six combinations
  TY <- quefts_tools(NutrUse_soilNPK)
  input$TargetYield <- TY$TargetYield
...
### Data partioning take 70:30 by country used to assess the random forest model 
...
#' @description filter the data with soil NPK, soil properties for iSDA and teh control yield in the way it fits to the random forest model and 
#' descritize the control yield 
#' @param soilNPKData a data frame with iSDA soil data, the control BLUP in kg/ha and soil NPK from the reverse QUEFTS
#' @param FCY_boundaries thresholds to classify the control yield in to classes as an indicator for very poor, average, high and very high fertile ground 
#' @param trialset true if the data for the NOT trial is being processed and FALSE if data is from intervention area
#' @param FCY 'none' if NOT trial dat ais processed, otherwise it is one of 'class1', 'class2', 'class3', 'class4', 'class5'
#' @return a data frame with only the variables required for the random forest model and control classes
rf_Datafilter <- function(soilNPKData, FCY_boundaries = c(7.5, 15, 22.5, 30), trialset = TRUE, FCY = 'none'){
  if(trialset == TRUE){
    
    soilNPKData$soilOM_0_20 <- soilNPKData$carbon_total_0_20 * 2
    soilNPKData$soilOM_20_50 <- soilNPKData$carbon_total_20_50 * 2
    
    
    soilNPKData <- soilNPKData[, c("soilN", "soilP", "soilK","aluminium_extractable_0_20", "aluminium_extractable_20_50",
    ...
    soilNPKData$Control <- soilNPKData$Control/1000
    soilNPKData$CONclass <- ifelse(soilNPKData$Control < FCY_boundaries[1], "class1",
                                   ifelse(soilNPKData$Control >= FCY_boundaries[1] & soilNPKData$Control < FCY_boundaries[2], "class2",
                                          ifelse(soilNPKData$Control >= FCY_boundaries[2] & soilNPKData$Control < FCY_boundaries[3], "class3",
                                                 ifelse(soilNPKData$Control >= FCY_boundaries[3] & soilNPKData$Control < FCY_boundaries[4], "class4", "class5"))))
    
    soilNPKData$CONclass <- as.factor(soilNPKData$CONclass)
    soilNPKData$Country <- as.factor(soilNPKData$Country)
    soilNPKData <- soilNPKData[complete.cases(soilNPKData), ]
    
  }else{
    Q <- c("soilN", "soilP", "soilK","aluminium_extractable_0_20", "aluminium_extractable_20_50",
            "bulk_density_0_20","bulk_density_20_50", "calcium_extractable_0_20", "calcium_extractable_20_50",
           "carbon_organic_0_20", "carbon_organic_20_50", "carbon_total_0_20", "carbon_total_20_50", "cation_exchange_capacity_0_20",
           "cation_exchange_capacity_20_50","clay_content_0_20","clay_content_20_50","iron_extractable_0_20","iron_extractable_20_50",
           "magnesium_extractable_0_20", "magnesium_extractable_20_50", "nitrogen_total_0_20", "nitrogen_total_20_50",
           ...
           "texture_class_20_50", "zinc_extractable_0_20", "zinc_extractable_20_50", "slope_angle", "fcc", "land_cover_2015", 
           "land_cover_2016","land_cover_2017", "land_cover_2018","land_cover_2019", "soilOM_0_20", "soilOM_20_50",
           "percentSOM_0_20", "percentSOM_20_50", "wp_0_20", "wp_20_50", "FC_0_20", "FC_20_50", "sws_0_20", "sws_20_50","Country")
    ...
    soilNPKData$CONclass <- as.factor(FCY)
    soilNPKData$Country <- as.factor(soilNPKData$Country)
    soilNPKData <- soilNPKData[complete.cases(soilNPKData), ]
...
#' Title
#'
#' @param maxInv max investment farmers are willing to invest on fertilizers
#' @param fertilizers a data frame as a result of function  fertilizerFunc
#' @param rootUP price of 1 ton cassava root
#' @param areaHa farm size
#' @param country currently working for RW, NG or TZ, if this is to be expanded it needs testing
#' @param FCY  farmers control yield among the five levels c(FCY1, FCY2, FCY3, FCY4, FCY5)
#' @param riskAtt adressing risk level famers want to take, conservative = profit must be 2*invest, risk lovers could go to profit=invest
#' @param crop  currently, cassava, maize, potato
#'
#' @return
#' @export
#'
#' @examples
FR_byLocation <- function(maxInv, fertilizers, rootUP, areaHa, country, FCY, riskAtt, crop){
  
  if(country == "Rw"){
    WLY_CY_FCY <- readRDS(paste("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/Output/RW_CY_", FCY, ".RDS", sep=""))
    SoilData <- readRDS(paste("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/Output/RW_", FCY, "_soilNPK.RDS", sep=""))
  }else if (country == "GH"){
    WLY_CY_FCY <- readRDS(paste("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/Output/Ghana_CY_", FCY, ".RDS", sep=""))
    SoilData <- readRDS(paste("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/Output/GH_", FCY, "_soilNPK.RDS", sep=""))
  }
 
  WLY_CY_FCY$water_limited_yield <- round(WLY_CY_FCY$water_limited_yield, digits = 0)
  WLY_CY_FCY$CurrentYield <- round(WLY_CY_FCY$CurrentYield, digits = 0)
  ## for every lat and lon
  SoilData$location <- unique(paste(SoilData$lat, SoilData$lon, sep="_"))
  WLY_CY_FCY$location <- unique(paste(WLY_CY_FCY$lat, WLY_CY_FCY$long, sep="_"))
  latlon <- unique(WLY_CY_FCY$location)
  
  ## for every planting date
  plDate <- unique(WLY_CY_FCY$pl_Date)
  
  ## For every harvest day: harvest is possible weekly between 8 and 15 months after planting 
  daysOnField <- unique(WLY_CY_FCY$daysOnField )
  Loc_FR_Recom <- NULL
  for (ll in latlon){
    ## get one row of WLYCY and soil data
    print(ll)
    WLY_CY_FCY_ll <- WLY_CY_FCY[WLY_CY_FCY$location == ll, ]
    SoilData_ll <- SoilData[SoilData$location == ll, ]
    ...
    for(pld in plDate){
      ...
      harvD_FR_Recom <- NULL
      for(hd in daysOnField){
        print(hd)
        WLY_CYll <- WLY_CY_FCY_ll_pd[WLY_CY_FCY_ll_pd$daysOnField == hd, ]
        
        
        FRadv <- getFRrecommendations(WLY_CYll=WLY_CYll, SoilData_ll=SoilData_ll, maxInv=maxInv, 
                                      fertilizers=fertilizers, rootUP=rootUP, areaHa=areaHa, country=country, 
                                      FCY=FCY, riskAtt = riskAtt, crop = "cassava")## TO DO : crop is a function in R change it here to something else
        ...
        FRadv_A$weekNr <- WLY_CYll$weekNr
        FRadv_A$daysOnField <- WLY_CYll$daysOnField
        
        harvD_FR_Recom <- rbind(harvD_FR_Recom, FRadv_A)
        # fname <- paste("FR", ll, "pld", pld, "hd", hd, sep="_")
        # saveRDS(harvD_FR_Recom, paste("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/Output/RW_FR_Recom/", fname, ".RDS" ,sep=""))
      }
      PlDatr_FR_Recom <- rbind(PlDatr_FR_Recom, harvD_FR_Recom)
    }
    fll <- fname <- paste("/FR", ll, sep="_")
    saveRDS(PlDatr_FR_Recom, paste("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/Output/", country, "/", FCY, fll, ".RDS" ,sep=""))
    
    Loc_FR_Recom <- rbind(Loc_FR_Recom, PlDatr_FR_Recom)
...
QUEFTS_Parallel <- function(WLY_CY_FCY_ll_pd, SoilData_ll, maxInv, fertilizers, rootUP, areaHa, country, FCY, riskAtt, crop){
  ...
  source("/home/akilimo/projects/AKILIMO_Modeling/QUEFTS/QUEFTS_Functions.R", local=TRUE)
...
# get_SoilNPK_AOI <- function(trainData, AIOdata, FCY){
#   AIOdata <- AIOdata[, colnames(trainData)]
#   trainData$purpose <- "Traindata"
#   AIOdata$purpose <- "Testdata"
#   
#   TT <- rbind(trainData, AIOdata)
#   
#   TT <-TT[!TT$slope_angle == "No data", ]
#   TT$slope_angle <- as.numeric(as.character(TT$slope_angle))
#   TT$fcc <- as.factor(as.character(TT$fcc))
#   TT$land_cover_2015 <- as.factor(as.character(TT$land_cover_2015))
#   TT$land_cover_2016 <- as.factor(as.character(TT$land_cover_2016))
#   TT$land_cover_2017 <- as.factor(as.character(TT$land_cover_2017))
#   TT$land_cover_2018 <- as.factor(as.character(TT$land_cover_2018))
#   TT$land_cover_2019 <- as.factor(as.character(TT$land_cover_2019))
#   
#   trainData_Rw_FCY <- TT[TT$purpose == "Traindata", ]
#   getData_Rw_FCY <- TT[TT$purpose == "Testdata", ]
#   
#   ## link the GPS for the test data 
#   getData_Rw_FCY <- unique(merge(getData_Rw_FCY, RW_soil[, c("lat", "lon", "NAME_1", "NAME_2", "index")], by="index"))
#   
#   Ndata_Train_RW_FCY <- subset(trainData_Rw_FCY, select=-c(soilP, soilK, purpose, index))
#   Pdata_Train_RW_FCY <- subset(trainData_Rw_FCY, select=-c(soilN, soilK, purpose, index))
#   Kdata_Train_RW_FCY <- subset(trainData_Rw_FCY, select=-c(soilN, soilP, purpose, index))
#   
#   Ndata_Valid_RW_FCY <- subset(getData_Rw_FCY, select=-c(soilP, soilK, purpose, index))
#   Pdata_Valid_RW_FCY <- subset(getData_Rw_FCY, select=-c(soilN, soilK, purpose, index))
#   Kdata_Valid_RW_FCY <- subset(getData_Rw_FCY, select=-c(soilN, soilP, purpose, index))
#   
...
#   ##soil N
#   set.seed(773)
#   RF_N1 <- randomForest(soilN ~ ., Ndata_Train_RW_FCY1 , importance=TRUE, ntree=1000)
#   Ndata_Valid_RW_FCY$soilN <- predict(RF_N1, Ndata_Valid_RW_FCY)
#   summary(Ndata_Valid_RW_FCY$soilN)
#   
#   
#   ###soil P
#   set.seed(773)
#   RF_P1 <- randomForest(soilP ~ ., Pdata_Train_RW_FCY , importance=TRUE, ntree=1000)
#   Pdata_Valid_RW_FCY$soilP <- predict(RF_P1, Pdata_Valid_RW_FCY)
#   
#   
#   ## soil K
#   set.seed(773)
#   RF_K1 <- randomForest(soilK ~ ., Kdata_Train_RW_FCY , importance=TRUE, ntree=1000)
#   Kdata_Valid_RW_FCY$soilK <- predict(RF_K1, Kdata_Valid_RW_FCY)
#   
#   RW_FCY_soilNP <- merge(Ndata_Valid_RW_FCY, Pdata_Valid_RW_FCY[c("lat", "lon", "NAME_1", "NAME_2", "soilP")], by=c("lat", "lon", "NAME_1", "NAME_2"))
#   RW_FCY_soilNPK <- merge(RW_FCY_soilNP, Kdata_Valid_RW_FCY[c("lat", "lon", "NAME_1", "NAME_2", "soilK")], by=c("lat", "lon", "NAME_1", "NAME_2"))
#   RW_FCY_soilNPK$FCY <- FCY
#   return(RW_FCY_soilNPK)
#   
# }
